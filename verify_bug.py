"""Verify: the $71M came from position_cost bug in deferred scale-in.
When T1 sells 90% but position_cost isn't reduced, scale-in in Pass 2
computes inflated entry_price, causing stop loss to sell at unrealistic prices.
"""
import sys, os
...
def simulate_day_buggy(picks, starting_cash, cash_account=False):
    """Two-pass with the position_cost BUG that produced $71M."""
    cash = starting_cash
    unsettled = 0.0
    ...
    for ts in all_timestamps:
        ...
        for st in states:
            ...
            if st["entry_price"] is not None:
                ...
                # Stop loss (uses entry_price — NOT original_entry_price)
                ...
                # Partial sells (uses entry_price, NOT original)
        ...
        # PASS 2: entries first, then scale-ins
        allocation_queue = []
        ...
        for item in allocation_queue:
            ...
            if action == "entry":
                ...
                if cash >= add_size and add_size > 0:
                    ...
                    # BUG: uses position_cost (includes already-sold T1 shares)
                    # instead of remaining_cost = remaining_shares * entry_price
...
for date_str in ALL_DATES:
    ...
    states, ending_cash = simulate_day_buggy(picks, rolling_cash, cash_account=is_cash)
    for s in states:
...
print(f"\n{'='*80}")
print(f"  BUGGY TWO-PASS (position_cost bug in scale-in)")
print(f"  SI 19%, gate ON, entry_price for exits")
print(f"{'='*80}")
print(f"  Final Cash: ${rolling_cash:>15,.2f}")
print(f"  Trades: {total_trades}  WR: {wr:.1f}%  SI: {scale_ins}")
print(f"\n  Target: $71,084,943 (old baseline)")
print(f"  Fixed two-pass: $69,452")
print(f"  Current code: $52,147 (SI 19%) / $90,837 (SI OFF)")
