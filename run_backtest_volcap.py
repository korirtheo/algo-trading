"""Run backtest with custom VOL_CAP_PCT override.

Usage:
    python run_backtest_volcap.py 6.0              # 6% vol cap, no charts
    python run_backtest_volcap.py 6.0 --charts     # 6% vol cap, with charts
"""
import sys
import os
import io
...
print(f"=== Backtest: trial {data['trial_number']} params, VOL_CAP={vol_cap}% ===")
print(f"  Score: {data['score']:,.0f} | PnL: ${data['total_pnl']:,.0f} | PF: {data['pf']}")
print(f"  Enabled: {data['enabled']}")
print()

sys.argv = ["test_green_candle_combined.py", "stored_data_combined"]
if not with_charts:
...
# Build overrides from JSON params
overrides = []
param_to_var = {
...
override_block = "\n# --- Optuna params + VOL_CAP override ---\n"
override_block += "\n".join(overrides)
override_block += "\n# --- End injection ---\n\n"
