"""
Verify root cause: original_entry_price in partial sell kills returns.
Test: patch the tranche 1 target to use entry_price (old behavior) instead
of original_entry_price, and see if $71M comes back.
"""
import sys, os, copy
...
def simulate_day_patched(picks, starting_cash, cash_account=False, use_original_for_partial=True):
    """Same as tf.simulate_day but with configurable partial sell base price.
    use_original_for_partial=True: current code (original_entry_price for tranche 1)
    use_original_for_partial=False: old behavior (entry_price for tranche 1)
    """
    # Just monkey-patch and call. Too much code to duplicate.
    # Instead, let's look at the specific effect by modifying N_EXIT_TRANCHES behavior.
    pass


# Simpler approach: override the partial sell target computation
# by temporarily changing the code behavior via a global flag.

# Instead of duplicating simulate_day, let's add a global flag and modify test_full.py
# behavior. But that's invasive. Let's just create a minimal simulation.

# Actually, the simplest test: set SCALE_IN_TRIGGER_PCT > PARTIAL_SELL_PCT by enough
# that even with original_entry_price, the behavior is similar to old code.
# In old code: scale-in raises entry_price, which raises partial sell target above
# the candle high, so partial sell doesn't fire.
# If we set PARTIAL_SELL_PCT high enough that it never fires on scale-in candles
# regardless of original_entry_price, we'd see the same effect.

# But the cleanest test is: what if we run with the OLD single-pass structure?
# Let me implement a minimal old-style simulate_day.

from zoneinfo import ZoneInfo
ET_TZ = ZoneInfo("America/New_York")

def simulate_day_old_style(picks, starting_cash, cash_account=False):
    """OLD-style single-pass simulate_day matching the initial commit structure.
    Key differences from current:
    1. Single pass (entry + exits in same loop, entry falls through to exits)
    2. No original_entry_price (all exits use entry_price)
    3. Scale-in is gated by not partial_sold
    4. No volume cap, no EOD_EXIT_MINUTES, no breakeven stop
    """
    cash = starting_cash
    unsettled = 0.0
    ...
    for ts in all_timestamps:
        for st in states:
            ...
            # === ENTRY (single-pass: inline, falls through to exits) ===
            if st["entry_price"] is None:
                ...
                if entered:
                    ...
                    cash -= position_size
                    # FALLS THROUGH to exit checks below (old behavior)
                else:
                    continue

            # === EXIT LOGIC (single-pass: runs on same candle as entry) ===

            if c_high > st["highest_since_entry"]:
                st["highest_since_entry"] = c_high

            # Scale-in (BEFORE partial sell, uses entry_price for trigger)
            if (tf.SCALE_IN and not st["scaled_in"]
                    and st["remaining_shares"] > 0
                    and not st["partial_sold"]):  # always gated (old behavior)
                ...
                if c_high >= trigger_price:
                    ...
                    if cash >= add_size and add_size > 0:
                        ...
                        st["entry_price"] = total_cost / total_shares  # raises avg entry
            ...
            # Stop loss (uses entry_price, which may be raised by scale-in)
            ...
            # 3-tranche exits (uses entry_price, NOT original_entry_price)
...
# Run tests
CONFIGS = {
    "OLD single-pass (SI 19%, gate ON)": {
        "SCALE_IN": 1, "SCALE_IN_TRIGGER_PCT": 19.0, "SCALE_IN_GATE_PARTIAL": True,
        "BREAKEVEN_STOP_PCT": 0.0, "ENTRY_CUTOFF_MINUTES": 0,
    },
    "OLD single-pass (SI OFF)": {
        "SCALE_IN": 0, "SCALE_IN_TRIGGER_PCT": 19.0, "SCALE_IN_GATE_PARTIAL": True,
        "BREAKEVEN_STOP_PCT": 0.0, "ENTRY_CUTOFF_MINUTES": 0,
    },
}

print(f"\n{'='*80}")
print(f"  ROOT CAUSE VERIFICATION")
print(f"  Testing OLD single-pass structure to reproduce $71M baseline")
print(f"{'='*80}")
...
for label, cfg in CONFIGS.items():
    ...
    for date_str in ALL_DATES:
        ...
        states, ending_cash = simulate_day_old_style(picks, rolling_cash, cash_account=is_cash)
        for s in states:
...
print(f"\n  --- Current two-pass code for comparison ---")

for label, cfg in [
    ("NEW two-pass (SI 19%, gate ON)", {"SCALE_IN": 1, "SCALE_IN_TRIGGER_PCT": 19.0, "SCALE_IN_GATE_PARTIAL": True, "BREAKEVEN_STOP_PCT": 0.0, "ENTRY_CUTOFF_MINUTES": 0}),
    ("NEW two-pass (SI OFF)", {"SCALE_IN": 0, "BREAKEVEN_STOP_PCT": 0.0, "ENTRY_CUTOFF_MINUTES": 0}),
]:
    saves = {}
    for k, v in cfg.items():
        saves[k] = getattr(tf, k)
...
print(f"\n  Expected: OLD single-pass SI 19% ~= $71M (reproduces old baseline)")
print(f"  If confirmed, the two-pass restructure + original_entry_price change")
print(f"  is the cause of the drop.")
